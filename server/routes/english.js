...
router.get('/delete',function(req,res){
    Del
        .delete(req,English,'english')
        .then(function(result){
            var location = "/" + result;
            res.send('<script>alert("게시글을 삭제 했습니다."); location.href="' + location + '";</script>')
        });
  // English.findByIdAndRemove(req.query.id,function(err){
  //   if(err){
  //     res.send(err);
  //   }else{
